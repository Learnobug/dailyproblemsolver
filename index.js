...
async function main() {
  try {
    const dailyQuestionData = await fetchWithRetry(
      "https://leetcode.com/graphql/",
      {
        headers: {
          ...
          "sec-gpc": "1",
          "sentry-trace":
            "0445d479d373429391f89a574ac2caf3-8ffb8c07bb881109-0",
    ...
    const question = dailyQuestionData.data.activeDailyCodingChallengeQuestion.question;
    const titleSlug = question.titleSlug;

    const solutionsData = await fetchWithRetry("https://leetcode.com/graphql/", {
      headers: {
        accept: "*/*",
        "accept-language": "en-US,en;q=0.9",
        authorization: "",
    ...
    const articles = solutionsData.data.ugcArticleSolutionArticles.edges;
        ...
        const submissionData = await fetchWithRetry(
          `https://leetcode.com/problems/${titleSlug}/submit/`,
          {
            ...
            body: JSON.stringify({
              ...
              question_id: question.frontendQuestionId, // Use the correct ID
